...
local refresh = require "kong.plugins.jwtrefresh.refresh"

local json = require "cjson"
local openssl_digest = require "resty.openssl.digest"
local openssl_hmac = require "resty.openssl.hmac"
local openssl_pkey = require "resty.openssl.pkey"
local asn_sequence = require "kong.plugins.jwtrefresh.asn_sequence"
...
local registered_claims = {
  ...
  exp = {
    ...
    check = function(exp,conf)
      ...
      -- TODO Check for token expiry less than 2min
      if exp - conf.refresh_timeremaining_second <= time() then
        -- hit get request with token
        refresh.refreshtoken(conf)
      end
    end
  }
}
...
function _M:verify_registered_claims(claims_to_verify,conf)
  ...
  for _, claim_name in pairs(claims_to_verify) do
    ...
    if type(claim) ~= claim_rules.type then
      ...
      local check_err = claim_rules.check(claim,conf)
      if check_err then
